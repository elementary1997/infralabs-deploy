# Экспорт и импорт курсов, модулей, уроков и упражнений

Это руководство описывает процесс экспорта и импорта курсов, модулей, уроков и упражнений в формате JSON.

## Экспорт курсов

### Базовый экспорт

Экспортирует все опубликованные курсы, модули, уроки и упражнения:

```bash
./scripts/export-courses.sh
```

Файл будет создан в `./exports/courses_export_YYYYMMDD_HHMMSS.json`

### С дополнительными опциями

**Включить файлы иконок (как base64):**

```bash
./scripts/export-courses.sh --include-files
```

**Включить неопубликованные элементы:**

```bash
./scripts/export-courses.sh --include-unpublished
```

**Указать путь к файлу:**

```bash
./scripts/export-courses.sh --output ./backups/my_courses.json
```

**Все опции вместе:**

```bash
./scripts/export-courses.sh --include-files --include-unpublished --output ./backups/full_export.json
```

### Использование Django management команды напрямую

```bash
docker exec infralabs_web python manage.py export_courses /tmp/export.json

# С опциями
docker exec infralabs_web python manage.py export_courses \
    /tmp/export.json \
    --include-files \
    --include-unpublished
```

## Импорт курсов

### Базовый импорт

Создает новые курсы, модули, уроки и упражнения:

```bash
./scripts/import-courses.sh ./exports/courses_export_20250110_120000.json
```

### Обновление существующих

Обновляет существующие элементы (по slug) вместо создания новых:

```bash
./scripts/import-courses.sh ./exports/courses_export.json --update
```

### Пропуск существующих

Пропускает элементы, которые уже существуют:

```bash
./scripts/import-courses.sh ./exports/courses_export.json --skip-existing
```

### Восстановление оригинальных ID

⚠️ **ВНИМАНИЕ:** Это удалит ВСЕ существующие курсы, модули, уроки и упражнения!

```bash
./scripts/import-courses.sh ./exports/courses_export.json --restore-ids
```

### Комбинированные опции

```bash
# Обновить существующие, пропустить если не найдены
./scripts/import-courses.sh ./exports/courses_export.json --update --skip-existing
```

### Использование Django management команды напрямую

```bash
# Копировать файл в контейнер
docker cp ./exports/courses_export.json infralabs_web:/tmp/import.json

# Импортировать
docker exec infralabs_web python manage.py import_courses /tmp/import.json --update

# Удалить временный файл
docker exec infralabs_web rm /tmp/import.json
```

## Формат JSON файла

Экспортированный JSON файл имеет следующую структуру:

```json
{
  "metadata": {
    "version": "1.0",
    "export_date": "2025-01-10T12:00:00Z",
    "include_files": false,
    "include_unpublished": false
  },
  "courses": [
    {
      "id": 1,
      "title": "Ansible Basics",
      "slug": "ansible-basics",
      "description": "Основы Ansible",
      "icon": "🎓",
      "icon_file_path": "course_icons/course_1.png",
      "icon_file_data": "base64_encoded_data...",  // только если --include-files
      "icon_file_name": "course_1.png",
      "order": 0,
      "is_published": true,
      "created_at": "2025-01-01T00:00:00Z",
      "updated_at": "2025-01-10T12:00:00Z",
      "modules": [
        {
          "id": 1,
          "title": "Getting Started",
          "slug": "getting-started",
          "description": "...",
          "difficulty": "beginner",
          "order": 0,
          "icon": "📚",
          "icon_file_path": "module_icons/module_1.png",
          "icon_file_data": "...",  // только если --include-files
          "estimated_hours": 5,
          "is_published": true,
          "lessons": [
            {
              "id": 1,
              "title": "Introduction",
              "slug": "introduction",
              "content": "Markdown content...",
              "order": 0,
              "xp_reward": 50,
              "estimated_minutes": 15,
              "is_published": true,
              "exercises": [
                {
                  "id": 1,
                  "title": "First Exercise",
                  "slug": "first-exercise",
                  "description": "...",
                  "instructions": "...",
                  "starter_code": "...",
                  "solution_code": "...",
                  "test_cases": [...],
                  "hints": [...],
                  "xp_reward": 100,
                  "difficulty": "easy",
                  "order": 0,
                  "time_limit_seconds": 300,
                  "max_attempts": 10,
                  "is_published": true
                }
              ],
              "resources": [
                {
                  "id": 1,
                  "title": "Documentation",
                  "resource_type": "link",
                  "url": "https://...",
                  "description": "...",
                  "order": 0
                }
              ]
            }
          ]
        }
      ]
    }
  ]
}
```

## Сценарии использования

### Миграция курсов между серверами

**На исходном сервере:**

```bash
# Экспорт с файлами
./scripts/export-courses.sh --include-files --output /tmp/full_courses.json

# Копирование файла на новый сервер
scp /tmp/full_courses.json user@new-server:/path/to/infralabs-deploy/exports/
```

**На целевом сервере:**

```bash
# Импорт
./scripts/import-courses.sh ./exports/full_courses.json

# Или с обновлением существующих
./scripts/import-courses.sh ./exports/full_courses.json --update
```

### Резервное копирование курсов

```bash
# Создать регулярный бэкап (добавить в crontab)
0 2 * * * cd /path/to/infralabs-deploy && ./scripts/export-courses.sh --include-files --output ./backups/courses_backup_$(date +\%Y\%m\%d).json
```

### Восстановление из бэкапа

```bash
# Найти нужный бэкап
ls -lh ./backups/courses_backup_*.json

# Восстановить
./scripts/import-courses.sh ./backups/courses_backup_20250110.json --restore-ids
```

### Синхронизация между окружениями

```bash
# На production сервере (экспорт)
./scripts/export-courses.sh --output /tmp/prod_courses.json

# На staging сервере (импорт с обновлением)
./scripts/import-courses.sh /tmp/prod_courses.json --update
```

## Важные замечания

### Foreign Keys и зависимости

- Курсы создаются первыми
- Затем модули (связь с курсами)
- Затем уроки (связь с модулями)
- Затем упражнения и ресурсы (связь с уроками)

Импорт автоматически обрабатывает эти зависимости.

### Уникальные ограничения

- `slug` должен быть уникальным для курсов
- `slug` должен быть уникальным для модулей
- `(lesson, slug)` должны быть уникальными для упражнений
- `(module, slug)` должны быть уникальными для уроков

При импорте существующие элементы будут обновлены или пропущены в зависимости от опций.

### ID восстановление

При использовании `--restore-ids`:
- Все существующие данные будут удалены
- Оригинальные ID будут восстановлены из файла
- Это полезно для полного восстановления структуры

⚠️ **Внимание:** Это удалит все существующие данные!

### Файлы иконок

- Без `--include-files`: только пути к файлам экспортируются
- С `--include-files`: файлы кодируются в base64 и включаются в JSON
- При импорте файлы автоматически восстанавливаются, если они включены в JSON

Файлы сохраняются в:
- Курсы: `media/course_icons/`
- Модули: `media/module_icons/`

## Troubleshooting

### Ошибка: duplicate key value violates unique constraint

**Решение:** Используйте `--update` или `--skip-existing`:

```bash
./scripts/import-courses.sh file.json --update
```

### Ошибка: Foreign key constraint failed

**Решение:** Убедитесь, что импорт выполняется полностью. Проблема может быть в порядке импорта. Команда автоматически обрабатывает зависимости, но убедитесь, что JSON файл не поврежден.

### Ошибка: File not found (для иконок)

**Решение:** При импорте без `--include-files` файлы должны уже существовать в media директории. Для полного переноса используйте `--include-files` при экспорте.

### Большой размер файла

**Решение:** При экспорте с файлами JSON может быть очень большим. Для больших экспортов:
- Используйте сжатие: `gzip exports/courses_export.json`
- Или экспортируйте без файлов и переносите media директорию отдельно

## Примеры

### Полный цикл экспорт-импорт

```bash
# 1. Экспорт с файлами
./scripts/export-courses.sh --include-files --output ./exports/backup.json

# 2. Сжатие (опционально)
gzip ./exports/backup.json

# 3. Копирование на другой сервер
scp ./exports/backup.json.gz user@server:/path/

# 4. На целевом сервере: распаковка
gunzip backup.json.gz

# 5. Импорт
./scripts/import-courses.sh backup.json
```

### Регулярное резервное копирование

Добавьте в crontab (`crontab -e`):

```bash
# Ежедневный бэкап в 2:00 ночи
0 2 * * * cd /path/to/infralabs-deploy && ./scripts/export-courses.sh --include-files --output ./backups/courses_$(date +\%Y\%m\%d).json

# Удаление бэкапов старше 30 дней
0 3 * * * find /path/to/infralabs-deploy/backups -name "courses_*.json" -mtime +30 -delete
```
